<!-- Hacker / Dark Profile Header -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=30&duration=2600&pause=700&color=39FF14&center=true&vCenter=true&width=720&lines=%3E+whoami;Disa+Tale+%7C+Reverse+Lover;%3E+Access+Granted.;Welcome+to+my+GitHub." alt="Typing SVG" />
</div>

<div align="center">
  <img src="https://github.com/DisaWdcba.png" width="150" style="border-radius: 50%; border: 2px solid #39FF14;" alt="DisaWdcba" />

  <h3>█ Disa Tale █</h3>
  <p>
    <code>非男娘</code> · <code>Reverse Lover</code> · <code>Penetration Lover</code>
  </p>

  <img src="https://komarev.com/ghpvc/?username=DisaWdcba&color=39FF14&style=flat-square&label=ACCESS+LOGS" alt="Profile views" />
</div>

---

## `~/about_me`

```bash
$ cat profile.txt
```

- `STATUS` 正在探索 **AIGC / 安全边界 / OPSEC**
- `STACK` 热爱 **Python / JavaScript / YARA**
- `FOCUS` Reverse Engineering / Security Research / Automation
- `CONTACT` 欢迎交流技术问题
- `BBS` [卡饭论坛 space-uid-1307239](https://bbs.kafan.cn/space-uid-1307239.html)

---

## `~/github_metrics`

<div align="center">

| Metric | Value |
|:---|:---:|
| `FOLLOWERS` | **8** |
| `PUBLIC_REPOS` | **12** |
| `TOTAL_STARS` | **20** |

</div>

<div align="center">

| Language | Usage |
|:---|:---:|
| `Python` | **~60%** |
| `JavaScript` | **~25%** |
| `YARA` | **~15%** |

</div>

---

## `~/popular_repositories`

<div align="center">

| Repo | Description | Lang | Stars |
|:---|:---|:---:|:---:|
| [**SimpleAIPainting**](https://github.com/DisaWdcba/SimpleAIPainting) | A simple and fast workspace for AIGC | `JavaScript` | ⭐ 14 |
| [**r0idamcp-for-codex**](https://github.com/DisaWdcba/r0idamcp-for-codex) | A fast way to install r0idamcp through Codex | `Python` | ⭐ 6 |
| [**yaraplus**](https://github.com/DisaWdcba/yaraplus) | 修改自官方 YARA 工具 | `YARA` | - |
| [**WhitePatchGui**](https://github.com/DisaWdcba/WhitePatchGui) | 白文件 Patch 的自用版本 | `Python` | - |
| [**an-simple-asynchronous-crawler**](https://github.com/DisaWdcba/an-simple-asynchronous-crawler) | Python 异步爬虫练手项目 | `Python` | - |

</div>

---

## `~/tech_stack`

<div align="center">

![Python](https://img.shields.io/badge/Python-0D1117?style=for-the-badge&logo=python&logoColor=39FF14)
![JavaScript](https://img.shields.io/badge/JavaScript-0D1117?style=for-the-badge&logo=javascript&logoColor=39FF14)
![YARA](https://img.shields.io/badge/YARA-0D1117?style=for-the-badge&logo=virustotal&logoColor=39FF14)
![GitHub](https://img.shields.io/badge/GitHub-0D1117?style=for-the-badge&logo=github&logoColor=39FF14)
![VS Code](https://img.shields.io/badge/VS%20Code-0D1117?style=for-the-badge&logo=visual-studio-code&logoColor=39FF14)

</div>

---

## `~/connect`

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-DisaWdcba-0D1117?style=for-the-badge&logo=github&logoColor=39FF14)](https://github.com/DisaWdcba)
[![卡饭论坛](https://img.shields.io/badge/BBS-卡饭论坛-0D1117?style=for-the-badge&logoColor=39FF14)](https://bbs.kafan.cn/space-uid-1307239.html)

</div>

---

<div align="center">

```text
┌──────────────────────────────────────────────┐
│ root@github:~# echo "Stay curious."          │
│ root@github:~# ./explore --reverse --opsec   │
│ status: online                               │
└──────────────────────────────────────────────┘
```

</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:000000,50:0D1117,100:39FF14&height=120&section=footer" />
</div>
